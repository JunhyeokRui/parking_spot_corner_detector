...
@torch.no_grad()
def run(yolo_weight='yolov5s.pt',  # model.pt path(s)
    ...
    refinement_model = resnet18()
    refinement_model.fc = nn.Sequential(nn.Linear(512,2),nn.Sigmoid())
    refinement_state = torch.load(refinement_weight)
    refinement_model.load_state_dict(refinement_state)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        print('img that goes to the model')
        print(img.shape)
        pred = model(img,
                     augment=augment,
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        plot_exact_point(refinement_model,xyxy, im0, label=label, color=colors(c, True), line_thickness=line_thickness)
                        if save_crop:
