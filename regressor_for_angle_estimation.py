...
writer = SummaryWriter('./angle_estimator_weights/{}'.format(args.run))


class cropped_dataset(Dataset):
    """Face Landmarks dataset."""

    def __init__(self, trials_to_select, selected_place, transform=None):
        """
        Args:
            ...
            transform (callable, optional): Optional transform to be applied
                on a sample.
        """
        self.data_dir = os.path.join('../new_refinement_data','images')
        self.label_dir = os.path.join('../new_refinement_data','coord_labels')
        self.transform = transform
        self.image_list = []
        self.label_list = []
        for this_trial in os.listdir(self.data_dir):
            if 'trial' in this_trial and this_trial[6] in trials_to_select:
                for this_place in os.listdir(os.path.join(self.data_dir, this_trial)):
                    if this_place == selected_place:
                        for this_image in os.listdir(os.path.join(self.data_dir, this_trial, this_place)):
                            if 'jpeg' in this_image:
                                self.image_list.append(os.path.join(self.data_dir,this_trial,this_place, this_image))
                                self.label_list.append(os.path.join(self.label_dir,this_trial,this_place, '{}.npy'.format(this_image[:-5])))
...
def main():
    ...
    trainset = cropped_dataset(train_trials , args.place, transform_train)
    trainloader = data.DataLoader(trainset, batch_size=args.train_batch, shuffle=True, num_workers=args.workers)
    testset = cropped_dataset(test_trials ,args.place, transform_test)
...
def train(trainloader, model, criterion, optimizer, epoch, use_cuda):
    ...
    t_loss =0
    total =0
    for batch_idx, (inputs, label) in enumerate(tqdm(trainloader)):
        ...
        # print(outputs.shape)
        # targets = torch.stack(angle,1).float()
...
def test_output_sample_images(testloader, model, criterion, epoch, use_cuda):
    ...
    for i in range(1,10,1):
        ...
        circ = Arrow(args.crop//2,args.crop//2, int(np.cos(outputs[i][0].cpu()) * 10),int(-np.sin(outputs[i][0].cpu())*10),1.0,color='b')
        ax.add_patch(circ)
...
def save_checkpoint(state, epoch, checkpoint='angle_estimator_weights', best=None):
